...
# #### Dimension expansion
# Repeat the dat 20 times, representing 20 channels and therefore we have a new dimension and permute the dimensions for convenience. In the future we will replace this with a direct multi-channel simulation.
...
# We want to decode `condition`, but control for the effect of `continuous`. 
f = @formula 0 ~ 1 + condition + continuous
time = range(0, 0.44, step=1 / 100) ## fake time
designDict = [Any => (f, time)]
m = Unfold.fit(UnfoldModel, designDict, evts, dat_3d; solver=b2b_solver);

results = coeftable(m);
results.estimate = abs.(results.estimate); ## back2back has no sign
results = results[results.coefname.!="(Intercept)", :] ## the intercept is hard to interpret
...
#
# We can see from the graph that b2b solver identifies regions where the signal can be decoded, taking into account the respective other factor
